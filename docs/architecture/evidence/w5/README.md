...
Os paineis de ordem de servico estiveram vazios ate a correcao de temporalidade.
Os contadores chegavam como cumulativos, que a New Relic nao soma: `count()`
devolvia as amostras com os atributos corretos, enquanto `sum()` e ate `max()`
devolviam zero. [service-fase1 #75](https://github.com/postech-software-architecture/workshop-service-fase1/pull/75)
fixou a temporalidade em delta e, depois do deploy, `processing.error.count`
passou a somar exatamente 3 — as tres transicoes invalidas executadas, o mesmo
valor que a condicao do alerta exige.

Os tres widgets ainda vazios dependem de sinais que esta execucao nao produziu:
duracao por etapa exige uma ordem atravessando execucao e entrega, erros de
integracao exigem falha de e-mail ou webhook, e a busca por `correlationId` no log
depende de a aplicacao registrar a requisicao em nivel informativo.
